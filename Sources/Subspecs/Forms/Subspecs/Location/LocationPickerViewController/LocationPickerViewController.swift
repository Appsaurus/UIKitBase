...
open class LocationPickerSearchViewController: SearchViewController<MKMapItem>, AsyncTaskDelegate{
    ...
    override open func createSearchResultsTableViewController() -> PaginatableTableViewController<MKMapItem> {
...
open class LocationPickerMapViewController: ConfigurableViewController<LocationPickerMapConfiguration, ViewControllerStyle>, AsyncTaskDelegate, UIGestureRecognizerDelegate, MKMapViewDelegate{
    ...
    lazy var currentLocationButton: BaseButton = {        
        return BaseButton(icon: MaterialIcons.My_Location, buttonLayout: ButtonLayout(layoutType: .titleCentered, marginInsets: .zero))
    }()
    ...
    func showCoordinates(_ coordinate: CLLocationCoordinate2D, animated: Bool = true) {
        let displayRegion = MKCoordinateRegion.init(center: coordinate, latitudinalMeters: config.resultRegionDistance, longitudinalMeters: config.resultRegionDistance)
