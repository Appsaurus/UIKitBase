...
open class FormTextField<ContentView: UIView, Value: Any>: FormField<ContentView, Value>, FormTextFieldProtocol where ContentView: FormFieldViewProtocol{
    ...
    open lazy var textField: UITextField = {
        return self.textFieldToValidate()
    }()
...
//    }
    ...
    weak public  var confirmsField: FormTextFieldProtocol?
    ...
    public func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        textFieldDelegates.forEach({ (delegate) -> () in
    ...
    override func runValidationTests() -> [ValidationFailure] {
        ...
        if let confirmationField = confirmationField {
            ...
            if confirmationText != textField.text{
                ...
                let explanationMessage = customErrorMessages[failureType] ?? "\(self.fieldName) field and \(confirmationField.fieldName) field must match."
                failures.append(ValidationFailure(failureType: failureType, explanationMessage: explanationMessage))
            }
        }
        ...
        case .tooLong:
            ...
            let explanationMessage = customErrorMessages[failureType] ?? "\(self.fieldName) field must be less than \(maxCharacterCountLimit) characters."
            failures.append(ValidationFailure(failureType: failureType, explanationMessage: explanationMessage))
        case .tooShort:
            let failureType =  ValidationFailureType.tooFewCharacters
            let explanationMessage = customErrorMessages[failureType] ?? "\(self.fieldName) field must be at least \(minCharacterCountLimit) characters."
            failures.append(ValidationFailure(failureType: failureType, explanationMessage: explanationMessage))
        case .justRight:
            break
        }
        ...
        if let illegalChars = illegalCharactersInText(textField.text!){
            ...
            let explanationMessage = customErrorMessages[failureType] ?? "\(self.fieldName) cannot contain \(illegalChars)"
            failures.append(ValidationFailure(failureType: failureType, explanationMessage: explanationMessage))
        }
        ...
        if let minNumberCount = minNumberCount, textField.text?.count(of: .decimalDigits) ?? 0 < minNumberCount{
            ...
            let explanationMessage = customErrorMessages[failureType] ?? "\(self.fieldName) field must have at least \(minNumberCount) numbers."
            failures.append(ValidationFailure(failureType: failureType, explanationMessage: explanationMessage))
        }
        ...
        if let minLetterCount = minLetterCount, textField.text?.count(of: .letters) ?? 0 < minLetterCount{
            ...
            let explanationMessage = customErrorMessages[failureType] ?? "\(self.fieldName) field must have at least \(minLetterCount) letters."
            failures.append(ValidationFailure(failureType: failureType, explanationMessage: explanationMessage))
        }
