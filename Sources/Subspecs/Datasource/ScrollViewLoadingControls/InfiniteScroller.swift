...
class InfiniteScroller: NSObject {
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        if context == &KVOContext {
            if keyPath == "contentSize" {
                ...
                case (.vertical, false):
                    distance = scrollView!.contentSize.height + defaultContentInset.bottom - point.y - scrollView!.frame.height
                case (.horizontal, true):
                    distance = scrollView!.contentSize.width - point.x - scrollView!.frame.width
                case (.horizontal, false):
                    distance = scrollView!.contentSize.width + defaultContentInset.right - point.x - scrollView!.frame.width
                }
    ...
    func containerFrame(scrollView: UIScrollView) -> CGRect {
        ...
        case (.vertical, false):
            return CGRect(x: 0, y: scrollView.contentSize.height + defaultContentInset.bottom, width: scrollView.bounds.width, height: defaultDistanceToTrigger)
        case (.horizontal, true):
            return CGRect(x: scrollView.contentSize.width, y: 0, width: defaultDistanceToTrigger, height: scrollView.bounds.height)
        case (.horizontal, false):
            return CGRect(x: scrollView.contentSize.width + defaultContentInset.right, y: 0, width: defaultDistanceToTrigger, height: scrollView.bounds.height)
        }
    }
