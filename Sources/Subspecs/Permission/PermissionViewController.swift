...
open class PermissionViewController: StackedAlertViewController, PermissionSetDelegate {
	...
	@discardableResult
	public class func authorize(permissions: Permission..., permissionViewModel: PermissionViewModel = PermissionViewModel(), from presenter: UIViewController, success: @escaping VoidClosure, failure: @escaping ClosureIn<PermissionSet>) -> Self?{
		guard PermissionSet(permissions).status != .authorized else{
			success()
			return nil
		}
		let permissionViewController = self.init(permissions: Array(permissions), permissionViewModel: permissionViewModel, onPermissionChange: { (vc, permissionSet, permission) in
